...
function maxLiquidityForAmount0Imprecise(
  ...
  const intermediate = sqrtRatioAX96.multiply(sqrtRatioBX96).divide(Q96)
  return bigInt(amount0)
    .multiply(intermediate)
    .divide(sqrtRatioBX96.subtract(sqrtRatioAX96))
}
...
function maxLiquidityForAmount0Precise(
  ...
  const numerator = bigInt(amount0)
    .multiply(sqrtRatioAX96)
    .multiply(sqrtRatioBX96)
  const denominator = Q96.multiply(sqrtRatioBX96.subtract(sqrtRatioAX96))
 ...
 */
function maxLiquidityForAmount1(sqrtRatioAX96: BigInteger, sqrtRatioBX96: BigInteger, amount1: BigintIsh): BigInteger {
  if (sqrtRatioAX96.greater(sqrtRatioBX96)) {
    ;[sqrtRatioAX96, sqrtRatioBX96] = [sqrtRatioBX96, sqrtRatioAX96]
  }
...
export function maxLiquidityForAmounts(
  ...
  if (sqrtRatioCurrentX96.lesserOrEquals(sqrtRatioAX96)) {
    return maxLiquidityForAmount0(sqrtRatioAX96, sqrtRatioBX96, amount0)
  } else if (sqrtRatioCurrentX96.lesser(sqrtRatioBX96)) {
    const liquidity0 = maxLiquidityForAmount0(sqrtRatioCurrentX96, sqrtRatioBX96, amount0)
    const liquidity1 = maxLiquidityForAmount1(sqrtRatioAX96, sqrtRatioCurrentX96, amount1)
    return liquidity0.lesser(liquidity1) ? liquidity0 : liquidity1
  } else {
