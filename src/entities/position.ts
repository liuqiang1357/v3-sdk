import { BigintIsh, MaxUint256, Percent, Price, CurrencyAmount, Token } from '@liuqiang1357/uniswap-sdk-core'
...
export class Position {
  ...
  private _mintAmounts: Readonly<{ amount0: BigInteger; amount1: BigInteger }> | null = null
  ...
  private ratiosAfterSlippage(
  ...
  ): { sqrtRatioX96Lower: BigInteger; sqrtRatioX96Upper: BigInteger } {
    ...
    if (sqrtRatioX96Lower.lesserOrEquals(TickMath.MIN_SQRT_RATIO)) {
      sqrtRatioX96Lower = TickMath.MIN_SQRT_RATIO.add(bigInt(1))
    }
    let sqrtRatioX96Upper = encodeSqrtRatioX96(priceUpper.numerator, priceUpper.denominator)
    if (sqrtRatioX96Upper.greaterOrEquals(TickMath.MAX_SQRT_RATIO)) {
      sqrtRatioX96Upper = TickMath.MAX_SQRT_RATIO.subtract(bigInt(1))
    }
    return {
      sqrtRatioX96Lower,
      sqrtRatioX96Upper
   ...
   */
  public mintAmountsWithSlippage(slippageTolerance: Percent): Readonly<{ amount0: BigInteger; amount1: BigInteger }> {
   ...
   * @returns The amounts, with slippage
   */
  public burnAmountsWithSlippage(slippageTolerance: Percent): Readonly<{ amount0: BigInteger; amount1: BigInteger }> {
    // get lower/upper prices
    const { sqrtRatioX96Upper, sqrtRatioX96Lower } = this.ratiosAfterSlippage(slippageTolerance)
   ...
   */
  public get mintAmounts(): Readonly<{ amount0: BigInteger; amount1: BigInteger }> {
